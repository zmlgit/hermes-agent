...
def _get_config() -> Dict[str, Any]:
    try:
        ...
        return load_config_readonly().get("plugins", {}).get("kanban_workflow", {})
    except Exception:
        return {}
...
def _get_notify_target() -> Optional[str]:
    cfg = _get_config()
    val = cfg.get("notify_target")
    return str(val).strip() if val else None

def _get_notify_profile() -> Optional[str]:
    cfg = _get_config()
    val = cfg.get("notify_profile")
    return str(val).strip() if val else None
...
def _on_post_tool_call(tool_name: str = "", args: Optional[Dict[str, Any]] = None, result: Any = None, **_unused: Any) -> None:
    ...
    try:
        ...
        try:
            ...
            target = _get_notify_target()
            if target and ":" in target:
                platform, _, chat_id = target.partition(":")
                if platform and chat_id:
                    # Avoid duplicate rows when notify_target matches an inherited parent sub.
                    existing = conn.execute(
                        "SELECT COUNT(*) FROM kanban_notify_subs WHERE task_id = ? AND platform = ? AND chat_id = ?",
                        (task_id, platform, chat_id),
                    ).fetchone()
                    if not existing or existing[0] == 0:
                        kb.add_notify_sub(conn, task_id=task_id, platform=platform, chat_id=chat_id, notifier_profile=_get_notify_profile())
                        log.info("auto-subscribed %s → %s:%s", task_id, platform, chat_id)
        finally:
            conn.close()
    except Exception as exc:
        log.warning("[Kanban Auto] notify failed for %s: %s", task_id, exc)
