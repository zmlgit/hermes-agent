...
def _install_ws_ip_failover(ws_client_module: Any) -> None:
    """Carried fix (b7be210ef4, re-applied on top of #73779 isolation).

    msg-frontier.feishu.cn DNS pool returns ~12 IPs of which ~3 are black-holes
    (TCP/TLS hang with no RST). websockets.connect tries resolved IPs sequentially
    under a single open_timeout, so a first-IP black-hole kills the whole connect.
    Wrap the already-isolation-dispatched ``websockets.connect`` with a per-IP
    4s try-each loop. Idempotent; installs once per process.
    """
    ...
    async def _connect_with_ip_failover(uri: str, **kwargs: Any) -> Any:
        ...
        for ip in unique_ips:
            try:
                ...
                return await asyncio.wait_for(real_connect(uri, **kw), timeout=4.0)
...
def _run_official_feishu_ws_client(ws_client: Any, adapter: Any) -> None:
    ...
    # Carried fix (a490032bdc, re-applied on #73779 isolation): the SDK receive loop
    # honors a flaky _auto_reconnect flag and raises into the asyncio void on mid-flight
    # closes — the task dies silently and this profile serves 0 inbound messages until
    # gateway restart. Patched loop ALWAYS disconnects + reconnects, logging loudly.
    from types import MethodType as _MethodType

    async def _patched_receive_message_loop(client: Any) -> None:
        try:
            while True:
                msg = await client._conn.recv()
                asyncio.get_running_loop().create_task(client._handle_message(msg))
        except Exception as e:
            ...
            log_fn(client._fmt_log("receive message loop exit, err: {}", e))
            try:
                await client._disconnect()
            except Exception as disconnect_err:
                ws_client_module.logger.warning(
                    client._fmt_log("disconnect after receive loop exit failed: {}", disconnect_err),
                )
            try:
    ...
    ws_client._receive_message_loop = _MethodType(_patched_receive_message_loop, ws_client)
...
class FeishuAdapter(BasePlatformAdapter):
    ...
    def _load_settings(extra: Dict[str, Any]) -> FeishuAdapterSettings:
        ...
        # profile's .env must govern its own adapter. See #86905 (upstream); carried admins /
        # default_group_policy parsing kept for group_rules auth (no upstream equivalent).
